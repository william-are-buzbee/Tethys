...
// It also takes POST /_bench/<name> from the sound bench (src/bench.js, #bench) and writes it to test/render/<name>,
...
http.createServer((req,res)=>{
  ...
  if(req.method==='POST'&&url.indexOf('/_bench/')===0){ // POST /_bench/<name> → test/render/<name>. The name is a plain file name and nothing else; no directories, no traversal
    ...
    if(!/^[A-Za-z0-9_.-]{1,80}$/.test(name)||name.indexOf('..')>=0||!/\.(wav|json|txt)$/.test(name)){res.writeHead(400);res.end('bad name');return;}
    const parts=[];let n=0;
