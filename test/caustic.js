...
// CAU_T/CAU_SOFT; CS 0 is a hard step), SPAN (metres across, 16), W (pixels, 512). Snaps to CAU_PX blocks as the shader does (v11.36.2). What it proved on 13 Sep: six sines are dots or stripes, thirty-two are
...
for(const dep of (process.env.DEP||'5,10,16').split(',').map(Number)){const img=Buffer.alloc(W*W*3);let cov=0;
  for(let y=0;y<W;y++)for(let x=0;x<W;x++){let px=x/W*span,pz=y/W*span;if(PX>0){px=(Math.floor(px/PX)+0.5)*PX;pz=(Math.floor(pz/PX)+0.5)*PX;}const H=[0,0,0];
