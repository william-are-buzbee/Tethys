...
// CAU_T/CAU_SOFT), SPAN (metres across, 16), W (pixels, 512). What it proved on 13 Sep: six sines are dots or stripes, thirty-two are
...
const T=num('CAU_TILE'),N=num('CAU_N'),HMAX=num('CAU_HMAX'),SUN=num('CAU_SUN'),CT=+(process.env.CT||num('CAU_T')),CS=+(process.env.CS||num('CAU_SOFT')),HI=num('CAU_HI');
const TEX=new Function('THREE','WIND_A','Q',util+'\n'+src.slice(a,b)+'\nreturn CAU_TEX;')(THREE,3.49,{cau:9});
console.log('rings',TEX.map(r=>r.L+'m x'+r.n).join(', '),' tile',T,'m',N,'texels  CT',CT,'CS',CS);
...
const W=+(process.env.W||512),span=+(process.env.SPAN||16),out=path.join(__dirname,'preview');if(!fs.existsSync(out))fs.mkdirSync(out);
for(const dep of (process.env.DEP||'5,10,16').split(',').map(Number)){const img=Buffer.alloc(W*W*3);let cov=0;
  for(let y=0;y<W;y++)for(let x=0;x<W;x++){const px=x/W*span,pz=y/W*span;const H=[0,0,0];
    for(const r of TEX){const g=Math.exp(-dep*dep*2*Math.pow(Math.PI*SUN/r.L,2));const s=tap(r.s,px,pz);for(let k=0;k<3;k++)H[k]+=s[k]*g;}
    const jc=dep*0.248,dj=(1-jc*H[0])*(1-jc*H[2])-jc*jc*H[1]*H[1],I=1/Math.max(Math.abs(dj),0.02);
    const st=Math.min(1,Math.max(0,(I-(CT-CS))/(2*CS))),ci=1+(HI-1)*(st*st*(3-2*st));if(ci>1+(HI-1)*0.5)cov++;
