...
const PIX=process.env.PIX===undefined?1:+process.env.PIX,PX=PIX?(num('CAU_PX')||0):0,T=num('CAU_TILE'),N=num('CAU_N'),HMAX=num('CAU_HMAX'),SUN=num('CAU_SUN'),CT=+(process.env.CT||num('CAU_T')),CS=+(process.env.CS||(PIX?0:num('CAU_SOFT'))),HI=num('CAU_HI'),CD=num('CAU_D')||1e9,DK=num('CAU_DARK')||0;
...
for(const dep of (process.env.DEP||'5,10,16').split(',').map(Number)){const img=Buffer.alloc(W*W*3);let cov=0,sum=0;
  for(let y=0;y<W;y++)for(let x=0;x<W;x++){let px=x/W*span,pz=y/W*span;if(PX>0){px=(Math.floor(px/PX)+0.5)*PX;pz=(Math.floor(pz/PX)+0.5)*PX;}const H=[0,0,0];const gi=Math.floor((((px+ox)/GS[0])%1+1)%1*GN)%GN,gj=Math.floor((((pz+oz)/GS[0])%1+1)%1*GN)%GN,gu=1-GS[1]*(1-GT.d[(gj*GN+gi)*4]/255);
    ...
    const st=CS>0?Math.min(1,Math.max(0,(I-(CT-CS))/(2*CS))):(I>=CT?1:0),ci=1+Math.exp(-dep/CD)*((HI-1)*(st*st*(3-2*st))-DK*(1-Math.min(I,1)));if(ci>1+(HI-1)*0.5)cov++;sum+=ci;
    const v=Math.round(Math.min(255,110*ci)),o=(y*W+x)*3;img[o]=v;img[o+1]=v;img[o+2]=v;}
  fs.writeFileSync(path.join(out,'caustic_'+dep+'m.png'),png(img,W,W));console.log(dep+' m: lines over '+(cov/(W*W)*100).toFixed(1)+'% of the floor, the drawn factor\'s mean '+(sum/(W*W)).toFixed(3));}
