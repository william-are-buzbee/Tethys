...
js+='\nglobal.__sn={set:(x,y,z)=>{player.pos.set(x,y,z);player.vel.set(0,0,0);camera.position.set(x,y+2,z+8);const ci=cellOf(x),cj=cellOf(z);for(let dj=-1;dj<=1;dj++)for(let di=-1;di<=1;di++)if(ci+di>=0&&cj+dj>=0&&ci+di<NCELL&&cj+dj<NCELL)loadChunkNow(ci+di,cj+dj);},'+
  'ground:(x,z)=>groundAt(x,z),lm:()=>LM,tide:()=>TIDE,n:()=>PN,'+
  'stats:()=>{const o={live:0,kinds:[0,0,0,0,0],alpha:0,size:0,fall:0,nan:0,ymin:1e9,ymax:-1e9};for(let i=0;i<PN;i++){const x=pp[i*3],y=pp[i*3+1],z=pp[i*3+2];if(x!==x||y!==y||z!==z)o.nan++;if(pk[i]===255)continue;o.live++;o.kinds[pk[i]]++;o.alpha+=ps[i*2+1];o.size+=ps[i*2];o.fall+=SN_K[pk[i]].fall;if(y<o.ymin)o.ymin=y;if(y>o.ymax)o.ymax=y;}'+
  'if(o.live){o.alpha/=o.live;o.size/=o.live;o.fall/=o.live;}o.share=o.live/PN;return o;},'+
  'W:(x,y,z)=>{snowSeed(0,x,y,z);const w=[];let W=0;for(let k=0;k<5;k++){w.push(+snowW(k,0,y).toFixed(3));W+=w[k];}return {W:+W.toFixed(3),w:w};}};';
...
const LM=__sn.lm();
const KN=['live','floc','silt','bubble','black'];
const sites=[
...
ok(sites[0].W.w[0]>sites[0].W.w[2]+sites[0].W.w[3]&&sh(sites[0],0)>0.35,'the lit water over the lagoon is mostly live (the water '+sites[0].W.w[0]+' vs silt '+sites[0].W.w[2]+' and bubbles '+sites[0].W.w[3]+'; the box '+(100*sh(sites[0],0)).toFixed(0)+'%)');
ok(sh(sites[1],3)>0.05,'the surf carries bubbles ('+(100*sh(sites[1],3)).toFixed(0)+'%)');
...
ok(sh(sites[5],1)>sh(sites[6],1)||sites[5].S.share>sites[6].S.share,'the thermocline holds more floc than the water under it');
