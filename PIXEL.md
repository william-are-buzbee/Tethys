...
**Status: designed 14 Sep 2026, the six open questions answered the same day (Decided, at the end). Pass A built as v11.40 the same day (CHANGELOG v11.40; DESIGN The light, "The de-res"), seen by the person ("literally zero complaints"); pass B built as v11.41 the same day (CHANGELOG v11.41), seen by the person ("AWESOME"; the rock strata binned and the switch split in v11.41.1: `pixel light` and `texels` are two rows); C and D not built. **The person's verdict (14 Sep, on v11.41.3): its own look, not one to scoff at, but the game's vibe is minimalist — filled in by space, not noise; undecided; done for now. Both looks stay, off by default. Possible later: a very subtle version of the texels, a small amount to touch up what reads as smooth (theirs to ask). **15 Sep 2026:** the texels need a few more passes at *higher* intensity first, then the tone-down — today the grain only makes sand look good; everything else looks weird. Not asked for now; the performance pass comes first.** The ask (the person, 14 Sep, with a screenshot of the pixel light at 20 m): "de-res the
...
- **B — the texels' pattern** (built v11.41; the custom option waits for D). The grain hash and the class table; rock strata, sand grain, the vein; the coats' `pattern` in the
...
- **A — the de-res** (above; built v11.40). **B — the pattern by rule** (built v11.41), the lab's controls, `test/preview.js PIX=1`. **C — the edges**: the sea
