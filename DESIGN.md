...
**v11.71: the player's speed, accel and turn are `derive`'s, as every animal's** (no preset carries a lock; soft-arm 8.1 / 3.12 / 5.13, finback 6.2 / 2.01 / 3.08 with `sprint` 1.75, coilshell 4.0 / 1.84 / 2.51), the contact mass is the creatures' rule (size cubed), and a preset keeps only what derive has no term for (`cam`, `jetImp`, `sprint`, `venom`, the ability).
`player.js`. Third person by default; `f` toggles first person (v11.18: the camera `F.nose`×scale + `FP_AHEAD` ahead of the nose, the body hidden, the surface-side rule kept). **The camera's field and arm (v11.48, `CAM_K` in scene.js):** `fov` 62° vertical in third person (~94° across at 16:9 — Subnautica's default, the number that made a 55 m reaper read as a shark your size on a flat screen: a wide field renders every animal at about half its true angular size and the screen reads as a window; in a headset the rendered angle is the real one), `fovFP` 62° in first person, `arm` 0 m added to the clade's `cam` (6.5–7.5) in third person. `applyCam` (player.js) sets the camera's field by mode, a compare when nothing changed. Live from the readout, `CAM_TUNE` (main.js): ←→ fov, pgdn-pgup fovFP, ↓↑ arm, Backspace resets with the fog's; the bestiary's arrows stand down while the readout is open. The choice is the person's (14 Sep 2026): 50–55 with the arm at +1.5–2 keeps the body's framing and grows a far animal by ~1.3×; a narrow field costs sight to the sides and can turn stomachs on a fast turn. Nothing is decided; the number in `CAM_K` is where it was. **The player is a spec (v11.68):** `player.clade` is `playerClade(spec, preset)` — speed, accel, turn, mass and `bite` from `statsOf(spec)` (derive with the spec's `stats` locks over it), `size` the spec's, `jet` and `legs` derive's; `CLADE_PRESETS` holds only what derive has no term for — `cam` (scaled by size against the preset's), `jetImp`, `sprint`, `venom` and the `ability` (ink, stun, withdraw; a spec of another clade than its preset's gets none). The three presets' old hand numbers are `stats` locks on `SPECS.soft/fin/coil` (soft 7.0/3.2/7/5/9, fin 8.8/2.6/4.5/7/26, coil 4.6/1.5/3/8/6 as speed/accel/turn/mass/bite), so they play as they did; derive alone would give the finback 7.5 m/s and 1.7 t — its calibration is loose. A spec that is no preset takes its fixed numbers from `presetFor` (a coiled ringmouth the coilshell, another ringmouth the soft-arm, anything else the finback). `test/player.js` checks the presets against the literal old numbers. `CLADES` — soft-arm (ink: predators lose you), finback (tail-strike stun), coilshell
