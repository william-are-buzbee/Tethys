...
**The island as a tuff cone (v11.86, 23 Sep 2026; world.js `CONE`, `coneK`, the `R.isle` block of `islandH`; chunks.js `terrainColor`, `placeCliffs`; flora.js `scrub`).** The person: the round 40 m hill with a plume on a round flat "looks too neat to feel like it could've been any island in the game" — a tutorial zone. A small vent erupting into shallow water on a shield's flank builds a tuff cone (Diamond Head, Koko Crater, Capelinhos), and its shape is the forces on it, which is what the hill lacked. Within 270 m of the cone's centre (nothing outside moves: 24,649 samples over the island's square bit-identical to v11.85): a Gaussian ring of radius `r` 70, inner width `wIn` 16 (a steep crater wall), outer `wOut` 60 (34° flanks), height `h` 30 × (1 + `lean` 0.6 × cos of the angle off downwind) — 48 m where the trades drop the ash (world 200°, the west-north-west rim; Diamond Head's high point is on its trade-wind rim), 12 upwind — × ±25% noise; breached on the upwind axis (`gapA` 0.8 rad fading from 0.36, the rim down to `gapH` −2.5: the sea broke in); the crater floor at `floor` 0.8 ± 0.5, a pond through the gap at every high tide, a salt pan at the low; the windward outer slope cut back to a cliff from r + 6 to r + 22 (`up`: 1 on the face the waves strike) over a wave-cut platform at `plat` −3 ± 1.2; the lee's flat of bars and pools at `flat` −0.6 ± 2.6 reaching `flatR` 120 all round, `spit` 80 further down-current and `leeR` 40 downwind, blended into the shelf over 80 m — the spit points down the current, which runs toward the caldera rim, and its 200 is v11.85's reach there, so the saddle between the cone and the rim (−13 to −2, 275–375 m from the spawn) is unchanged. The cone has its own exposure (`expoW` blended to 0.5 − 0.5·cos within 240 m: 1 on its windward face, 0 in its lee) and its ring is rock (`coneRock` = smooth(2, 8, the ridge's height) → `sub` ≥ 0.85 there: the boulder apron and the cliff kit's ledges, which `placeCliffs` tints tuff on the cone by `coneK`). Tuff altered by hot water is palagonite, tan to ochre: `terrainColor` blends the land to `tuff` 0.62/0.50/0.32 by `coneK` × smooth(1.5, 4, h), from the strand up, the same rule on the far terrain. The scrub keeps to `expo` ≤ 0.65 (salt-blasted off the windward rim; it grows in the crater and on the lee). Cold: the fumarole and the vog of v11.17.1 are struck (The sky, The mist) — a monogenetic cone erupts once. Land within 300 m: 2.9 ha → 6.9 ha (the ring's flanks). Seen: `test/render/v86_before_*.png` / `v86_after_*.png` — from the air the ring, the green crater floor with its pond, the breach with surf in it, the tan rims and their ledges; from the rim of the caldera a lower, longer, tan island where the round hill stood; underwater to windward a rock-strewn platform, to the lee the flat. Knobs to expect questions on: `h` (the size of the whole thing follows it: a 30 m rim needs a 190 m foot at 34°), the scrub's stalks on the lee flat, the ledges' size on the inner wall.
...
**The cloud field and the weather's clocks (v11.87, CLOUDS.md; clouds.js, world.js `weatherAt`).** The deck is one field, `cldN(w,f,fz)`, written twice — GLSL for the dome, the sea's topside and the land in air, JavaScript for the beam at the player and the lumps — from a float32-exact hash (Ashima's permute), so they agree: 3D value noise, the first octave plain and three billowed (gains 0.5/0.225/0.10/0.045), in the wind's frame, stretched along the wind `CLD.street` 0.5 × the trades with rows `CLD.rowL` 4200 m apart (`rowA` 0.07; a calm dissolves them), the tops leaning downwind `CLD.lean` 0.16, the vertical scale `CLD.zk` 0.55, a cap over every island whose land stands into the deck (`CLD_CAPS`: the giant, a Gaussian `capR` 3000×1800 m centred `capOff` 1200 m downwind of the summit, strength `CLD_S.cap` 0.22 at night to 0.42 by late afternoon × the trades). The threshold is the cover's quantile of the field (`CLD_TH`, 21 quantiles sampled at load; `cldThOf`), so the cover is the cloud fraction; the density `smoothstep(th+rise·fz², +CLD.edge 0.05)`, `rise` 0.30 less 60% when the tops spread. **The weather's clocks:** `day` = cos on the local hour, +1 at 5 h (two and a half hours before sunrise), −1 at 20 h — on the cover (+0.06) and the shower threshold (−0.025); `wave` = the easterly waves, a ~4-day noise −1..1 — on the cover (+0.10), the showers (−0.035), the wind (+0.05), the congestus (`deep` +0.3); the season on the cirrus (−0.35·s: the still season's, present 75% of its hours against 38%, means 0.40 / 0.12), the congestus (+0.5 still) and the haze (`hazeK` 1.5 still); `spread` (a full deck without rain toward dawn and in the morning) flattens the tops. The year: showers 8.5% of hours (dawn 0.125 against the afternoon's 0.06), calms 27%, mean cover 0.44, boot cover 0.53 with the first shower 3.25 h in (`test/sky.js`). **The beam at the player** (`cloudAt` → `K.cloudHere`, eased over ~0.7 s): `sunL` = dayK·(1−`CLD_SHADE` 0.85·cloudHere)·(1−0.7 rain), the moon's beam ×(1−0.8·cloudHere); the caustics, shafts and shadows follow. **The shadows** (`cloudSh`, `Q.cldSh` 1/0): the surface's direct light × the base's transmission at the fragment over the player's (exact for the direct term, capped ×4); the land in air darkened by the beam's share where it stands under more cloud than the player. **The lumps** (the `near clouds` switch): `LUMP_N` 24 heaps of 14–21 dodecahedra at the field's maxima within `LUMP_R` 2400 (a scan of `LUMP_CELL` 300 m cells every `LUMP_T` 0.5 s, 0.6 ms), measured by a walk (`LUMP_STEP` 100 × `LUMP_WALK` 14, ≥220 m), keyed by their cell, grown by scale at `LUMP_EASE` 0.5/s, drawn in the horizon pass under the deck's near fade `CLD.near` 1400–2600; five fixed lumps a cap; the lumps stand down as the cover closes (v11.88: × smooth 0.82→0.5 of the cover — a plateau of cloud gets one lump, so an overcast's near sky was a hole). **The showers (v11.88, CLOUDS.md §6; clouds.js `SHWR`, `CELLS`, `updateCells`):** the weather's `rain` is the trigger; an episode (its start hour ×4) births one cell `SHWR.ahead` 6000 m upwind of the player, across the wind by a hash in ±`SHWR.band` 4500, radius `SHWR.R` 2200–3200, steered along the wind at `SHWR.u` 2.2 × `WIND_U` (in the field's frame: that less the field's drift), alive `SHWR.life` 800 real s (grown over `grow` 120, gone over `fade` 220), `SHWR.n` 2 at once; its congestus `CLD.bump` 0.5 × a Gaussian at its anchor in the field (`cellK`; the deck's `T` × (1+2·cellK), a lump's height the same); the rain here = its strength × smooth(1.15→0.5 of `SHWR.shaft` 0.5 R, the distance); `cellNear` = its strength within three radii (the cover +0.25·cellNear, the bow); the curtain in `SKY_FS` (the ray's path through the shaft's cylinder to the base, α = (1−exp(−path/`SHWR.vis` 2500))·strength·smooth toward the axis, the colour the shade ×0.9 toward the horizon by distance and the mist to its distance, the far haze × (1−α)); the bow × max(curtain, rain here). Uniforms `uCellA`/`uCellB` (the anchor, 1/R², the strength). Uniforms: `uCld` (the drift x,z, the threshold, cloudHere), `uCldB` (the streets, the cap, the rise, the base's height) shared through ShaderLib; the dome's own `uDeckC`/`uDeckL` (the deck's sunlight: the keyframe at the sun's altitude plus 0.8° of depression, dimmed 45% at the horizon; lit until 0.8° under), `uLump`.

**The cloud volume (v11.89, CLOUDS.md §8; clouds.js `CLD_ATLAS`, `cldBake`, `cldFT`, `cldDensT`; atmosphere.js `VOL_GLSL` `cloudVol`).** The deck is a density in three dimensions, marched: the noise tiles at `CLD_ATLAS.per` 16 base cells (a period per octave, lacunarity 2), baked once by the GPU into 32 slices of 256² (2048×1024, half float or bytes; z 0–`zmax` 0.6) and read with the tile inset a texel and the height mixed between slices; the density at a world point is the field's (the wind's frame, the streets, the lean, the caps, the cells) at the height in the *local* deck (`uVolT.x` × (1+2·cellK)), thresholded at `th + rise·fz²` over `CLD.edge`. The march: the slab's entry and exit (the base `uCldB.w` to the deepest top `uVolT.y`; a camera inside starts at itself), up to `Q.vol` 80 steps (0 on the low tier: the slices) of `max(VOL_DT[0]` 30 m`, t·VOL_DT[1]` 0.018`)` from an ordered 4×4 dither, to `VOL_FAR` 16 km; α per step `1−exp(−d·dt·VOL_SIG` 1/120`)`; at every step with cloud the light from two samples toward the luminary at `VOL_L1` 120 and `VOL_L2` 300 m, `exp(−(l1·120+l2·180)·VOL_SIGL` 1/150`)`; the sun's share `sS/sA` quantised `floor(q·3+0.5)/3`, the ambient `0.85+0.3·(mean fz)` of a shade `mix(zen·0.5+hor·0.45, grey 0.55, 0.35)` by day; `col = mix(shd·amb, lit, q) + deckC·fw·(1−α)·1.2`, fogged toward the horizon by `1−exp(−mean t/9000)`·0.85; α × the horizon fade × the lumps' near fade. v11.90: the volume is the only clouds — `cloudDeck` (the slices), the scud, the lumps and their switches are struck; `Q.march` 80 / 28 (the key was `Q.vol` for one version and shadowed the master volume: 80× gain); `CLD.sc` 1/750; the ambient 0.95–1.2 of a shade half way to a light grey; `CLD.base` 0.12 rounds the bottom (the threshold + 0.8·rise·((base−fz)/base)² under it); `FZ0` 0.1.

**The dome (`sky`, `SKY_FS`; rewritten v11.17).** *(v11.87: the deck's slices read the field above, each its own shape; the thickness darkening is smooth (`thick`), the lighting samples the field 180 m toward the luminary at gain 12 (doubled as the sun lowers), a high sun lights the tops, a low one the bases (0.40·(1−fz)·(1−sunUp)·deckL) and the sides; the lit colour `mix(nightLit, dayLit, max(uDay,uDeckL))` with `dayLit` toward `uDeckC`·1.3 by 0.9·deckL; a silver lining `uDeckC·pow(d·lum,16)·(1−c)·1.2`; the cirrus on a coarse field (`fbm(cp·0.30)`, `0.70−0.30·cirrus` over 0.16) for where it is and a fine one (`0.50–0.76`) for the fibres, its unlit colour `zen·0.55+hor·0.15` — darker than the twilight — and the overcast sky's grey under rain. The v11.17 numbers below that these replace stand as the record.)* One `ShaderMaterial` on the camera-following dome, shown above water, and the
...
**The mist above the water (`HAZE`, `MIST_DAWN`, world.js; `updateHaze`, atmosphere.js; `MIST_GLSL`, scene.js; v11.17; the vog and the fumarole `VOG`/`FUME` struck v11.86 — the cone is a cold tuff cone, World shape).** What the climate
...
`terrainColor`'s colours — through a camera from `HZ_NEAR` (20 m) to 80 km — the land mesh itself only past `uHzMin` = `FAR` × `HZ_SEA0` (v11.86.1: nearer, its coarse triangles can stand over the camera in a hollow the fine ground has and the mesh does not — the tuff cone's crater — and nothing in the main pass covers the sky; the sea disc starts there too and the far regions reach past the far plane, so the handover is covered); then the main pass over it with the depth cleared. The planet's curvature
...
jelly), drag measured against the rest's own motion *except* `SWAY` (0.5) of the part that is not the body's own travel (v11.80: the body's translation is carried whole, so a steady swim carries the arms with it and a squid swimming arms-first does not stream backwards, but the body turning under them and the stroke moving them are water they push — a tip sits damp/ks × SWAY × that speed, 0.09 s of it, behind its rest for as long as a curve lasts: the finback's petals 0.11–0.23 m through a 69°/s curve against 0.07 cruising. The body's travel is read off the frame's origin, `o.rigO`, not `o.vel` — they need not agree), the segment lengths kept from
...
**The swarms (v11.84, PLANKTON.md §5, §10, pass 4; creatures_defs.js `DEFS.swarm`, `swarmBuild`, `cropK`; ecology.js `ecoCap`; chunks.js `placeKind`; creatures_ai.js `updateSwarm`, `swarmDepth`, `swarmPig`; atmosphere.js the swarm block).** The grazers of the water column as one thing: a cloud 8–14 m across (`SW_R` by the swarm's draw q), a ledger record like any kind — a `SPAWN` row of 2.5 a cell whose capacity is the envelope × `cropK` of the lit standing stock (world.js `cropAt`: half at 0.5 mg/m³, so 1.7 a cell at the ring, 2.2 on the fed flank, 0.3 in the basin), rates by hand (`r` 0.5, `cycle` 2: size 6 through `bioMass` would breed it like a ridge) — with no body: no geometry (`swarmBuild` an empty group; the LOD, the shadow list and the shared geometry are all empty and fine), no hit capsule (nothing bites it), no push (`ghost` skips `bodyPush`), no orientation. It is placed in the open water over its place (`openY` 10–40 up) with its pigment kind read off the crop there (`swarmPig`: gold on the flank, green in the lagoon). **Its day** (`swarmDepth(floor, dayK, moonL, q)`): the night layer `SW_NIGHT` −28 (−14·q), the day layer `SW_DAY` −380 (−60·q) or the floor + `SW_FLOOR` 6 where the floor comes first (over the shelf the grazers hide near the bottom by day), a full moon holding the night layer `SW_MOON` 45 m deeper (Tethys' moon gives 0.28 of noon: −80 under it), mixed by the sky's `dayK` so the transit rides the dusk and the dawn; the cloud climbs at `SW_RISE` 4 m/s real (a 300 m transit in ~80 s, one game hour), wanders within `home` 90 at `SW_WANDER` 0.35 and is carried by the current. **Drawn by the snow**: the last `SN_SW` (480, a quarter of `PN` on low) points of the snow's buffer are the swarm block; every half second `swAssign` gives `SN_SWP` 80 points each to the `SN_SWN` 6 swarms nearest the camera within `SW_SEE` 160 (`swBlock`: offsets in a flattened cloud, the kind's colour `SW_COL` a shade warmer than the snow's, size `SW_SZ` 1.7×, `pk` 254); in the loop those points relax to their place in the cloud (a slow jitter on `swSeed`) after the same `FLOW` scatter the snow takes, so a body parts them; a block whose swarm is gone parks at the camera. No new material, no new draw call, no points beyond the budget: the snow's own loop runs to `PN_SN`. Seen (`test/render/v84_swarm_beside.png`): a gold cloud at night over the shelf edge beside the kelp, dense against the snow. Headless (`test/plankton.js`): the layers −35 / −393 / −80 under a full moon / the floor over a 40 m shelf; 31 swarms in the loaded cells at the ring. Food since v11.85 (Filter feeding, below): `edible`, so mortal on the ledger; its biomass (`bioMass` 216 t) is `c.flesh`, eaten through, the cloud drawing the share its flesh leaves (`swarmShare`) and the ledger counting it so (`ecoWriteBack`); a swarm the ledger owes a loaded cell forms out of sight whole (`ecoTick` with `placeKind` `off`, no clutch, no juvenile). Not yet: grazing the crop down (the capacity is the crop's; a swarm eating the map would need the map to change).

**Filter feeding (v11.85, PLANKTON.md §11, pass 5; creatures_spec.js `derive.filter`, the `sieve` switch on the `comb` part and the `net` arms; creatures_defs.js `defPhysics`; ecology.js `ecoOf`; creatures_ai.js `FILTER`, `filterIntake`, `filterSeek`, `filterPlayer`, `swarmR`, `swarmShare`, `swarmSpent`, `seeOf`).** A body's sieve is an area off its build: the frontal `combs` (two combs of n segments × seg × the teeth's tl: the comb 13.3 m², the sifter 0.08), a `comb` part marked `sieve` (the rake's plane w × len, or n hanging plates: the ram's 1.2) or a webbed `net` marked `sieve` (the funnel's mouth, π(R + len·wsp/2)²: the veil's 99.8 at s 1.4); with the square of the scale, and 0 on a body with none. The switch is the creator's, not the geometry's: the hingeshells' mouth combs rake the floor (the tread, the cinder, the wedge, the plough, the lash's) and the pall's net catches in the dark below the chemocline, where no swarm is — a geometric rule made every one of them a hunter of swarms on paper and would have starved the pall. **On the ledger** a kind with any sieve has the swarm on its prey (`ecoOf`, never on `DEFS.prey`: nothing chases, holds or bites a swarm, which has no capsule) and is a hunter — the type-III response, the condition, starvation, `ke`; the swarm is `edible` and so mortal, breeding at its hand `r` 0.5 toward the crop's capacity and taken by biomass share. The 120-day census: the swarms at 17,064 of 19,549; the sifter 19,737 → 20,965, the comb 237 → 288, the ram 160 → 205; nothing under a fifth. (The veil at hp 1e9 is still immortal on the paper and sits at `ke`, the capacity its swarms allow.) **Live**, the intake a second is `FILTER.eff` 0.35 × the sieve × the water through it (the body's speed, floored at the combs' own sweep `FILTER.sweep` 1 m/s) × the swarm's density (`c.flesh` — `bioMass` 216 t at full — over the cloud's volume, 4/3·π·R³ × 0.45 flattened, R `swarmR` 8–14 by q), capped by what the stomach has room for and what the swarm has left; off the stomach over the kind's meal as a kill's is, off the swarm's flesh, and a brooding ringmouth takes nothing (`feeds`). A starving veil hanging still in a full swarm is fed in ~45 s and takes half of it; the density falls with the flesh, so the tail is exponential — under a starving veil the last of a swarm takes ~140 s more — and at `FILTER.spent` 8% of its biomass the swarm disperses (`swarmSpent`: the ledger debited, `POP.eaten`). The snow draws the share of a swarm's points its flesh leaves (atmosphere.js, `swarmShare`), so the cloud thins as it is worked through; `ecoWriteBack` counts a half-eaten swarm as half. **The seek**: a filter feeder past `ECO.hungry` looks for the nearest swarm within `seeOf` — `FILTER.see` 120 + its `home` (the veil 320, the comb and the ram 300, a sifter school 150; a cloud tens of metres across is found from further than a fish is) — every `FILTER.scan` 0.5 s, swims to it at its cruise, circles inside at 0.7 of that through wander points in the cloud (`filterSeek`, state `filter`), and lets it go fed to `FILTER.full` 0.06 or when it is spent; the wander role (the veil, the comb) and the ram's idle between hunts go through it, and the wander role runs `hungerTick` now when it has a sieve. A sifter school moves as one: `updateSchools` reads the members' mean hunger, takes the swarm as the school's target and, feeding in it, as its home, so the ribbons follow the layer's day; each member takes its own intake (`updateBoid`). A swarm the ledger owes a loaded cell forms out of sight, whole (`ecoTick` through `placeKind` with `off`; no clutch, no juvenile — `juvDef` has no spec to scale). The player: see The player, The stomach. `test/plankton.js` has the numbers; `test/live.js` tables the filter feeders with the hunters (a swarm as a meal, `filter` as hunting). Costs: a scan every half second over the creatures for the swarms per hungry filter feeder or school, the intake a distance check a frame.
...
right angle left and away, w with space a 39° climb, space alone straight up and never a flip; **s is the reverse** (v11.79, the person, 22 Sep 2026: "S needs to be a reverse. You need to be able to swim backwards and use the controls fully") — every body backs along its own facing without turning, at `STEER.rev` (jet 0.6, sculling 0.35, legs 0.5 × what the legs are) of its speed, and while it is still going forward that same thrust is the brake (`STEER.brake` 2.5 × accel); not every animal will back for ever — the person: "later on that can be a cognitive capacity"; v11.77's turn keys struck; space jumps or hops a walker); derive's buoyancy is a drift the pitch trims (`BUOY_V` sinks −0.25, neutral 0,
floats 0.15 m/s).
...
looks along the heading from the nose. **Nothing about the animal enters the camera's orientation** (v11.80): the look is the heading's, dipped by the constant `dip` = atan(lift·(1−`CAM_AIM`)/(arm+3)) — 9.4° for the finback — with its up composed from that same dipped angle. Until then it was a `lookAt` at the *body* from a position that lags it, so a sidestep swung the view after the animal: `a` alone rolled the camera 3° off the heading with the mouse untouched, and rocking a and d rocked the picture (the person: "awkward jittery … it also rotates the camera … like a lack of antialiasing"). The three effects that move the camera on their own — the hurt shake, the bite nudge, the fov kick — are the effects list's `camera shake` (`FX.camFx`). **The camera sits over the body, not on its axis** (v11.79, the person on v11.78: "the camera does seem to be a bit directly behind you … contributing to the racing game feeling"): `CAM_LIFT` 0.42 of the arm above the body's line (the finback 3.15 m at an arm of 7.5 — a share, so a 21 m sickle is looked down on as much as a 3 m finback, where 1.4 m flat put every camera on the body's own axis) and the look raised by `CAM_AIM` 0.45 of that lift, which tilts the view down 9° and leaves the animal about a sixth of the frame below centre with its back and its shape in view. **Free look** (v11.77.3, the effects list's `freeLook`, **the default since v11.79** — the person, 22 Sep 2026: "free look should be the default for sure … the game needed those simpler controls and a camera that isn't pulling you like it's a racing game"; `FX_V`/`FX_NEW` in effects.js carry a changed default past a browser that has already saved its choices): the heading unclamped and the body holding its facing until a movement key is down, then turning into the keys' direction; the thrust runs by the body's
...
only (main.js, `hungerLine`): no bar by the text rule; how the body itself might show it is the person's (CHANGELOG v11.73, Unseen 1). **v11.85: a body with a sieve** (`derive.filter` > 0 on its line kind — the veil, the comb, the sifter, the ram, or a comb-built child) is a hunter of the swarms on the model (ecology.js `ecoOf`) and runs the same clock; its meal is a swarm passed through (creatures_ai.js `filterPlayer` from `updatePlayer`, `filterIntake`: the nearest swarm within 60 m every half second, the intake while inside its cloud), and the readout's line says the sieve's m², whether you are in a swarm and how much of it is left. A founder with neither prey nor sieve (a grazer) still has no stomach on the model.
