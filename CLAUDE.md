...
| atmosphere.js | the sea surface, the sky (v11.87: the deck and the cirrus read clouds.js's field; the deck's own sunlight `CLD_S.deckC`; the beam at the player by `cloudHere`), rain, haze, light shafts, marine snow (v11.84: the swarm block — the last `SN_SW` points ride the nearest swarms, `swAssign`/`swBlock`), fog and light by medium, HUD, compass |
...
| clouds.js | the cloud field (v11.87, CLOUDS.md): one function of a point on the deck in GLSL (`CLD_GLSL`, `cloudSh` — in every fogged material's fog chunk) and JavaScript (`cldN`, `cloudAt`), the threshold by the cover's quantile (`CLD_TH`), the streets, the caps over tall islands (`CLD_CAPS`), the state `CLD_S` and its uniforms `FOG_CLD`/`FOG_CLDB`; the lumps — the near clouds as low-poly heaps at the field's maxima (`lumpScan`, `updateClouds`) and the giant's cap, drawn in the horizon pass; the showers (v11.88): cells with a place (`SHWR`, `CELLS`, `updateCells` — born upwind on the weather's trigger, steered along the wind, a bump in the field, the rain at the camera by distance, the curtain in the dome) |
...
| `CLOUDS.md` | the sky's clouds and the weather's clocks (24 Sep 2026; passes A–C and the lumps built v11.87, pass D — showers with a place — v11.88; storms next): what was there and what was artefact, what this planet's sky is by the day, the wave and the season, the giant's cap, the field written twice, the lumps' judgement for the person; §7 the person's answers |
