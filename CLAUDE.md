...
| `test/sky.js` | the cloud field and the weather (v11.87, CLOUDS.md): the threshold table monotone, the cloud fraction equal to the cover asked for, no NaN over the world with the giant's cap, the beam's shadow in range; the year's clocks — showers ~8% and twice as likely before dawn as in the afternoon, the cirrus the still season's, calms a quarter, boot fair with a shower a few hours in; the showers (v11.88: one cell an episode, born upwind, its speed and growth, the field over it, rain over the player and none three radii off); the costs | `node test/sky.js` |
| `test/plankton.js` | the plankton field (v11.81): the crops by kind and the front's X at the doc's sites, the crops at three depths by the clock, the tint; no NaN across the world, the encoding in range, the doc's numbers (a gyre, island water, the fed flank, the lee's eddy, the lagoon green, the flank gold, no bloom at the vent), the vertical, the ring on a side radial and broken on the axis and moving in at neaps, the maps against the field, the GLSL from the tables, the storm's pulse bounded; the swarms (v11.84): the capacity by the crop, the day's layers; filter feeding (v11.85): the sieve off the build for the four and none on the tread, the pall or the finback, the swarm on the paper prey and on no chase list, a starving veil fed through a full swarm and the swarm's loss, nothing taken fed or outside the cloud, the swarm dispersing eaten down, a hungry comb seeking a swarm 50 m off and letting it go fed | `node test/plankton.js` |
| `test/combat.js` | seeded since v11.93 (the same run each time); §18 the edge's work (v11.93: the verdict's ratios against the agreed §7 table, the sickle tearing a tail off and the hood failing to, the stone's swallow as a sequence, the stamina's rest, the arms' cone, the bill's edge line); holds form and kill, the joint never parts (v11.92: no rope — the grip and the struck point are one; the mouth bites what it touches; a chase turns at its rate, commits and overshoots; no orbit; the petals to the skin), the player held, bled, pinned and killed, the grab, the bite, the sting, the paralysis, the blood trail, the miss rule, the poison, a tail torn off and the stump growing back; a table of every hunter of the player and its outcome; the matrix (v11.54): every hunter at contact behind each prey — the covering under the hold, the edge's verdict, the gape, the jaws' distance | `node test/combat.js` |
...
| `test/census.js` | the ecology on paper: capacities, rates, the model for N days; fails if a kind collapses under a fifth (v11.85: loads the spec compiler too, so every kind's sieve is on its row and the filter feeders hunt the swarms on paper) | `node test/census.js 120` (`--test` runs 40) |
| `test/live.js` | the ecology where the player is: four game days of clutches laid and hatched with cells loaded, then a table of every hunter's (and, v11.85, every filter feeder's — a swarm is its meal) hunger against the distance to its nearest meal | `node test/live.js`, `TIER=low …` |
...
| atmosphere.js | the sea surface, the sky (v11.87: the deck and the cirrus read clouds.js's field; the deck's own sunlight `CLD_S.deckC`; the beam at the player by `cloudHere`; v11.89: the deck as a ray-marched volume, `cloudVol`, the light banded after the cloud; v11.90: the only clouds, the slices struck), rain, haze, light shafts, marine snow (v11.84: the swarm block — the last `SN_SW` points ride the nearest swarms, `swAssign`/`swBlock`), fog and light by medium, HUD, compass |
...
| clouds.js | the cloud field (v11.87, CLOUDS.md): one function of a point on the deck in GLSL (`CLD_GLSL`, `cloudSh` — in every fogged material's fog chunk) and JavaScript (`cldN`, `cloudAt`), the threshold by the cover's quantile (`CLD_TH`), the streets, the caps over tall islands (`CLD_CAPS`), the state `CLD_S` and its uniforms `FOG_CLD`/`FOG_CLDB` (the lumps of v11.87, low-poly heaps in the horizon pass, are struck since v11.90); the showers (v11.88): cells with a place (`SHWR`, `CELLS`, `updateCells` — born upwind on the weather's trigger, steered along the wind, a bump in the field, the rain at the camera by distance, the curtain in the dome); the volume (v11.89): the noise tiled and baked by the GPU into an atlas (`CLD_ATLAS`, `cldBake`), the density at a world point (`cldDensT`) that atmosphere.js `cloudVol` marches; `updateClouds` is the bake |
...
| `GLOBE.md` | the planet, then the region (25 Sep 2026, designed, not built): continents far away, a slow plate, a pulsing plume, 10–20°, ice ages; the chain revised into a hotspot cluster with a seamount province; the planet-map tool and the region window; §7 the person's answers, §8 the build order — read before placing any island |
...
| `CLOUDS.md` | the sky's clouds and the weather's clocks (24 Sep 2026; passes A–C and the lumps built v11.87, pass D — showers with a place — v11.88, the volume §8 v11.89, the only clouds since v11.90; storms next): what was there and what was artefact, what this planet's sky is by the day, the wave and the season, the giant's cap, the field written twice, the lumps' judgement for the person; §7 the person's answers |
...
- **Creatures.** `SPECS[id]` → `compile` (a core builds the frame, parts merge into the body or become rigs; `derive` computes
  ...
  → `updateCreatures` dispatching on role (boid, trap, watch, hunter, graze, wander, coil, ambush, drift, sail, swarm), then contact. A `swarm` (v11.84) is a kind with no spec, no body and no capsule (`swarmBuild`, `ghost`), drawn by the snow's swarm block; its `SPAWN` row's `crop` flag makes its capacity the plankton's (ecology.js `ecoCap` × `cropK`). Since v11.85 it is food: mortal, its biomass `c.flesh`, eaten through by any body whose build has a sieve (`derive.filter` → `DEFS.filter` by `defPhysics`; `ecoOf` puts the swarm on that kind's paper prey, never on `DEFS.prey`; creatures_ai.js `FILTER`, `filterIntake`, `filterSeek`) — a swarm is never chased, held or bitten.
...
- **Combat** (combat.js): a fight is a hold — a rope between a grip (jaws, arms, claws, read off the spec by `compile`) and the
  ...
  its pin time is a pin (v11.93: subdued — the struggle under half the grip for a second, `SUBDUE`; the struggle fades with the held body's stamina, `STAMINA`) and the placed act follows from the verdict (swallowed by the gape — since v11.93 a sequence, `SWALLOW`; opened, skewered, crushed, the nerve cord)
...
- **Quality tier** `Q` (scene.js) is numbers only, never code paths; `#low`/`#high` force it; low is automatic on small touch screens. (`Q.cldSh` 0 leaves the cloud shadows out of the shaders on the low tier, v11.87; `Q.march` is the cloud volume's steps — never `Q.vol`, the master volume's key, which v11.89 shadowed for one version at 80× gain.)
...
- **Units and frames.** 1 unit = 1 m, `GRAV` 9.8, y up, sea level 0, east +x, north −z, angles in radians from +x toward +z.
  **All creatures face +z.** An arms or claws predator's `reach` must exceed the contact distance of its and its prey's hit capsules or it can
  never land a bite; a jaw's bite is its mouth touching a capsule (v11.92, combat.js `mouthOn`), and its `reach` is only the strike range's floor. A day is 30 game hours in 40 real minutes; rates are per game day.
...
- The stub's `setTimeout` runs immediately; `location.hash` comes from `TIER`. Since v11.92 the stub's `Matrix4.lookAt`, `Quaternion.setFromRotationMatrix` and `slerp` are real and an object's matrix composes from its quaternion: headless creatures turn. A body placed by hand faces +z until something turns it; set the player's facing with `faceQ(P)` after its yaw (test/combat.js `choose`).
