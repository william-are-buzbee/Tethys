...
| `test/sky.js` | the cloud field and the weather (v11.87, CLOUDS.md): the threshold table monotone, the cloud fraction equal to the cover asked for, no NaN over the world with the giant's cap, the beam's shadow in range; the year's clocks — showers ~8% and twice as likely before dawn as in the afternoon, the cirrus the still season's, calms a quarter, boot fair with a shower a few hours in; the lumps' scan (the pool, every lump measured, the caps); the showers (v11.88: one cell an episode, born upwind, its speed and growth, the field over it, rain over the player and none three radii off); the costs | `node test/sky.js` |
...
| clouds.js | the cloud field (v11.87, CLOUDS.md): one function of a point on the deck in GLSL (`CLD_GLSL`, `cloudSh` — in every fogged material's fog chunk) and JavaScript (`cldN`, `cloudAt`), the threshold by the cover's quantile (`CLD_TH`), the streets, the caps over tall islands (`CLD_CAPS`), the state `CLD_S` and its uniforms `FOG_CLD`/`FOG_CLDB`; the lumps — the near clouds as low-poly heaps at the field's maxima (`lumpScan`, `updateClouds`) and the giant's cap, drawn in the horizon pass; the showers (v11.88): cells with a place (`SHWR`, `CELLS`, `updateCells` — born upwind on the weather's trigger, steered along the wind, a bump in the field, the rain at the camera by distance, the curtain in the dome); the volume (v11.89): the noise tiled and baked by the GPU into an atlas (`CLD_ATLAS`, `cldBake`), the density at a world point (`cldDensT`) that atmosphere.js `cloudVol` marches |
...
| `CLOUDS.md` | the sky's clouds and the weather's clocks (24 Sep 2026; passes A–C and the lumps built v11.87, pass D — showers with a place — v11.88, the volume §8 v11.89; storms next): what was there and what was artefact, what this planet's sky is by the day, the wave and the season, the giant's cap, the field written twice, the lumps' judgement for the person; §7 the person's answers |
...
- **Quality tier** `Q` (scene.js) is numbers only, never code paths; `#low`/`#high` force it; low is automatic on small touch screens. (`Q.vol` 0 and `Q.cldSh` 0 leave a shader function out of the string, v11.87–89: the low tier keeps the slice deck and no cloud shadows.)
