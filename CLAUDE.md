...
| player.js | the player as a spec (`playerClade`, v11.68) and `CLADES` (the three presets), movement in water / air / on land, camera, abilities, damage, ink, splashes; the stomach (v11.73: `player.hunger` on the ledger's model through creatures_ai.js `hungerTick`/`kill`/`eatAt` by `eaterK`, `EAT`, the readout's `hungerLine`) |
...
| line.js | the line across generations (v11.69, LINEAGE.md §13.1–2): the lives on the slot and their tracks, the finback's clutch (`x`, `playerLay`), the editor at conception and its budget (v11.72, §13.4: `conceiveOpen`, `conceivePrice`, `BUDGET`, `conceiveClose`; the parent out of the world meanwhile, v11.72.1: player.js `playerAway`, `playerGone()`; the clutch paid from the stomach, v11.73: `LINE.egg`, `clutchCost`, `clutchHunger`), the broods as records that survive an unload (`BROOD_SURVIVE`), the young as a kind per spec (`lineKind`), the handover at death (`lineNext`, `lineContinue`), the sparkle (`starPath`, `updateSpark`) |
