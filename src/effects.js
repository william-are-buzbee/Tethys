...
const FX_LIST=[['caustics','caustics'],['pixel','pixel light'],['texels','texels'],['shadows','shadows'],['statics','world shadows'],['ground','ground shadows'],['sharp','sharp shadows'],['shafts','light shafts'],['snow','marine snow'],['rain','rain'],['clouds','clouds'],['shimmer','surface glow'],['blood','blood'],['vignette','vignette']];
const FX_DEF={caustics:true,cauK:1,pixel:false,texels:false,shadows:Q.tier!=='low',statics:false,ground:false,sharp:false,shafts:true,snow:true,rain:true,clouds:true,shimmer:true,blood:true,vignette:true}; // shadows: off on the low tier (a phone) until measured there; statics, ground (v11.30): the world's own shadows — the flora, rock and structures, and the terrain — off until measured (scene.js updateShadowS); sharp: both shadow maps at twice the size (scene.js shadowSize)
...
function fxApply(){vigEl.style.display=FX.vignette?'':'none';pixU.value=FX.pixel?1:0;texU.value=FX.texels?1:0;if(typeof wasAbove==='boolean')applyFog(wasAbove);shadowDirty();} // a switch changes the world map's caster set // the fog's light block reads FX (LIGHT_K); before the first frame applyFog runs on its own
