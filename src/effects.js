...
// freeLook (v11.77.3, the person: an option for the old camera): the mouse spins the camera round the body, which holds its facing until a movement key is down and then turns toward the camera (player.js); off, the mouse steers the animal and the camera is held within HEAD_MAX of it. The one switch here that is not a drawing.
// A switch is one line to add: a key in FX_DEF, a row in FX_LIST, and the system reading FX.key where it draws.
const FX_LIST=[['freeLook','free look'],['caustics','caustics'],['refract','refraction'],['pixel','pixel light'],['texels','texels'],['texelLight','banded light'],['shadows','shadows'],['statics','world shadows'],['ground','ground shadows'],['sharp','sharp shadows'],['shafts','light shafts'],['plight','own light'],['snow','marine snow'],['rain','rain'],['clouds','clouds'],['blood','blood'],['debris','debris'],['flush','flush'],['vignette','vignette']];
const FX_DEF={freeLook:false,caustics:true,cauK:1,pixel:false,texels:false,texelLight:true,shadows:Q.tier!=='low',statics:false,ground:false,sharp:false,shafts:true,snow:true,rain:true,clouds:true,blood:true,debris:true,flush:false,vignette:true,plight:true,refract:true,refrK:1}; // shadows: off on the low tier (a phone) until measured there; statics, ground (v11.30): the world's own shadows — the flora, rock and structures, and the terrain — off until measured (scene.js updateShadowS); sharp: both shadow maps at twice the size (scene.js shadowSize)
...
const FX_SLIDERS={caustics:['brightness','cauK',0,2,0.05],refract:['wobble','refrK',0,3,0.05]}; // refract (v11.51): the underside's refraction of the frame (atmosphere.js REFR_*), wobble a multiplier on REFR_K // a slider under a switch (v11.38.1, the person's ask): [label, key, min, max, step]; the key is a number in FX_DEF and persists with the switches
