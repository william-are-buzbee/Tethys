...
function hzUniforms(){const L=(THREE.ShaderLib&&THREE.ShaderLib.lambert&&THREE.ShaderLib.lambert.uniforms)||{},u={};for(const k of ['uFogC','uFogR','uFogW','uFogP','uFogS','uFogT','uFogA','uFogAC','uMist','uMistC','uMistW','uWaterMap','uFloorMap','uFogTC','uSkA','uSkB','uSkC','uSkD','uSkE','uSkF'])if(L[k])u[k]=L[k];return u;}
...
const HZ_SEA_MAT=hzMat(HZ_VS_HEAD+'void main(){vec4 wp=modelMatrix*vec4(position,1.0);vec2 dd=wp.xz-uFogC.xz;wp.y-=dot(dd,dd)*'+(0.5/HZ_R).toExponential(6)+';vec4 mv=viewMatrix*wp;vFogDepth=length(mv.xyz);vFogPos=wp.xyz;gl_Position=projectionMatrix*mv;}',
  HZ_FS_HEAD+THREE.ShaderChunk.fog_pars_fragment+'\nvoid main(){float d=vFogDepth;vec3 rd=(vFogPos-uFogC)/max(d,1e-3);float cm=max(-rd.y,0.0);float R=pow(1.0-cm,3.0)*0.85;vec3 Rw=reflect(rd,vec3(0.0,1.0,0.0));Rw.y=max(Rw.y,1e-3);Rw=normalize(Rw);'+
  'vec3 body=vec3(0.06,0.16,0.20)*mix(0.15,1.0,uSkA.w)*'+HZ_LIGHT[2].toFixed(3)+'/0.35;vec3 col=body*(1.0-R)+skyLite(Rw)*R;col=fogAir(col,uFogC,rd,d,1.0);gl_FragColor=vec4(dithering(col),1.0);}'); // the surface's topside rule at its far end (atmosphere.js SURF_MAT: the Fresnel weight on the mean normal, the sky reflected, the body under it): at grazing 85% the sky a few degrees up, darker than the horizon's band — the sea line
