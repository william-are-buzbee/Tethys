...
const Q=(function(){
  ...
  return tier==='low'
    ...
    :{tier:tier,pr:1.5,far:1600,flora:1.0,creatures:1.0,lights:4,phong:true,aa:true,budgetMs:6,farMs:3,farQ:12,surf:192,lodNear:1.0,rockLvl:2,target:7.5,casters:16,shafts:4,cau:5,hrtf:1,vol:1,cloud:5,snow:1800};
...
const SKYLITE_GLSL='vec3 skyLite2(vec3 d,float shade){vec3 uZen=uSkA.xyz,uHor=uSkB.xyz,uGlow=uSkC.xyz,uSunC=uSkD.xyz,uSun=uSkE.xyz,uMoon=uSkF.xyz;float uDay=uSkA.w,uCover=uSkB.w,uMoonL=uSkC.w;float up=d.y;float cs=dot(d,uSun);float k=smoothstep(-0.04,0.55,up);vec3 col=mix(uHor,uZen,k)*(0.92+0.10*cs*cs);'+
  ...
  'col*=1.0-0.35*uCover*shade;float disc=smoothstep('+Math.cos(SUN_R*1.3).toFixed(6)+','+Math.cos(SUN_R*0.85).toFixed(6)+',cs)*uDay*(1.0-0.9*uCover);col=mix(col,uSunC*1.4,disc);'+
  ...
  'vec3 skyLite(vec3 d){return skyLite2(d,1.0);}\n'+ // the window's and the reflection's sky: the deck's mean shade stands in for the clouds it cannot draw
  'vec3 skyFar(vec3 rd,float cy){float hm=1.0-exp(-mistFar(cy,rd.y));vec3 hc=uMistC.rgb*(1.0+uMistC.w*pow(max(dot(rd,uFogS.xyz),0.0),6.0));return mix(skyLite2(rd,0.0),hc,hm);}\n'; // the fog's far colour: no deck shade — the dome draws its clouds where they are and its gradient between them is unshaded, so a far thing must converge to the unshaded gradient or it stands as a silhouette 15% darker than the dome beside it (seen: the cone from 1500 m)
