...
const surfaceU={uAmp:{value:WAVE_AMP},uUnder:{value:1},uSkyR:{value:new THREE.Vector3(0.62,0.76,0.86)},uWin:{value:new THREE.Vector3(1,1,1)},uGlint:{value:new THREE.Vector3(1.0,0.97,0.88)},uRain:{value:0},uBody:{value:SURF_BODY}};
// uUnder: 1 when the camera is under water; uSkyR: the sky the
...
// The sky as the window sees it (v11.43): the sky shader's gradient, aureole, horizon glow, sun disc and glare, the moon's disc and halo, and the
// deck's shade without its noise — a function of a world direction, with the sky's own uniforms. SKY_FS is the reference; when its gradient or
// glare changes, change this with it.
const SKYLITE_GLSL='vec3 skyLite(vec3 d){float up=d.y;float cs=dot(d,uSun);float k=smoothstep(-0.04,0.55,up);vec3 col=mix(uHor,uZen,k)*(0.92+0.10*cs*cs);'+
  ...
  'col*=1.0-0.35*uCover;float disc=smoothstep('+Math.cos(SUN_R*1.3).toFixed(6)+','+Math.cos(SUN_R*0.85).toFixed(6)+',cs)*uDay*(1.0-0.9*uCover);col=mix(col,uSunC*1.4,disc);'+
  'col+=uSunC*(pow(max(cs,0.0),300.0)*0.7+pow(max(cs,0.0),10.0)*0.13)*uDay*(1.0-0.85*uCover);'+
  'float cm=dot(d,uMoon);float md=smoothstep('+Math.cos(MOON_R*1.15).toFixed(6)+','+Math.cos(MOON_R*0.9).toFixed(6)+',cm)*(1.0-0.9*uCover);col=mix(col,vec3(0.95,0.93,0.85)*(0.15+0.85*min(1.0,uMoonL*3.6))*(1.0-0.6*uDay),md);col+=vec3(0.80,0.85,0.95)*0.22*uMoonL*(1.0-uDay)*exp(-(1.0-cm)*2600.0);return col;}\n';
const SURF_MAT=(function(){
  ...
  m.onBeforeCompile=function(sh){
    sh.uniforms.uTime=timeU;sh.uniforms.uChop=chopU;sh.uniforms.uAmp=surfaceU.uAmp;sh.uniforms.uUnder=surfaceU.uUnder;for(const k of ['uZen','uHor','uGlow','uSunC','uSun','uMoon','uDay','uCover','uMoonL'])sh.uniforms[k]=skyU[k];sh.uniforms.uSkyR=surfaceU.uSkyR;sh.uniforms.uWin=surfaceU.uWin;sh.uniforms.uGlint=surfaceU.uGlint;sh.uniforms.uRain=surfaceU.uRain;sh.uniforms.uBody=surfaceU.uBody;sh.uniforms.uFogP={value:FOG_PSURF}; // the window's sky (v11.43): the sky shader's own uniforms, shared. FOG_PSURF (v11.42): the surface is the boundary — its ray is fogged in the camera's medium whole (scene.js). v11.44: for one version this comment sat mid-line and ate the six bindings after it — the surface's fog set fell back to the split one and its own crests were classed under water: dark patches along every far crest from above
    sh.vertexShader='uniform float uTime;uniform sampler2D uFloorMap;attribute float aSpace;varying float vH;varying vec2 vWp;varying vec3 vNup;varying float vBrk;\n'+WAVE_DEP_GLSL+WAVE_GLSL+sh.vertexShader.replace('#include <begin_vertex>',
      '#include <begin_vertex>\n{vWp=transformed.xz+modelMatrix[3].xz;vec2 dw=waveDep(vWp,modelMatrix[3].y);float hh=waveH(vWp,uTime,aSpace,dw);transformed.y+=hh;vH=hh;vBrk=waveBrk(vWp,dw);vNup=normalMatrix*vec3(0.0,1.0,0.0);}'); // the mesh sits at the tide (modelMatrix[3].y), so the water depth is the floor map's plus that (v11.44); vBrk: the breaking excess, the foam // vNup: the mean surface's normal (up) in view space — the Fresnel and the window are judged against it, not the facet (v11.6)
    sh.fragmentShader='uniform float uTime;uniform float uAmp;uniform float uUnder;uniform vec3 uZen,uHor,uGlow,uSunC,uSun,uMoon;uniform float uDay,uCover,uMoonL;\n'+SKYLITE_GLSL+'uniform vec3 uSkyR;uniform vec3 uWin;uniform vec3 uGlint;uniform float uRain;uniform float uBody;uniform float uChop;uniform mat3 uFogR;varying float vH;varying vec2 vWp;varying vec3 vNup;varying float vBrk;float snell=1.0;float cv2=1.0;\n'+sh.fragmentShader
      .replace('#include <normal_fragment_begin>','#include <normal_fragment_begin>\n{vec3 V=normalize(vViewPosition);float cv=abs(dot(V,normal)),cm=abs(dot(V,normalize(vNup)));cv2=cv;'+
        ...
        'else if(gl_FrontFacing){float fr=pow(1.0-cm,3.0),fm=max(smoothstep(0.62,0.98,vH/uAmp)*0.5*uChop,vBrk)*0.85,k=fr*0.85*(1.0-fm);diffuseColor.rgb=diffuseColor.rgb*(1.0-k)*(1.0-fm);totalEmissiveRadiance=totalEmissiveRadiance*uWin+uSkyR*k+vec3(0.88,0.92,0.92)*0.9*uWin*fm;'+ // foam (v11.44): the breaking excess (vBrk, the surf) and the crests' whitecaps (by height, halved and by the chop — a wind sea's, not the swell's, which shoaling now raises over the whole shelf), *emissive* in the sky's light (uWin), not lit by the facet: far off the flat normal comes from screen derivatives across triangles under a pixel quad and lit foam went dark — dark streaks along every far crest from the beach (seen 14 Sep)
        'diffuseColor.a=max(mix(uBody,1.0,fr),fm);'+ // the reflected sky is emissive (a reflection is not lit by the sun) and the surface is opaque at grazing angles (v11.3); the body's alpha is uBody (v11.42), the foam opaque
