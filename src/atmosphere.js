...
const surfaceU={uRefr:{value:refrTex},uRefrOn:{value:0},uRefrRes:{value:new THREE.Vector4(1,1,1,1)},uRefrK:{value:REFR_K},uAmp:{value:WAVE_AMP},uUnder:{value:1},uSnell:{value:0},uWin:{value:new THREE.Vector3(1,1,1)},uGlint:{value:new THREE.Vector3(1.0,0.97,0.88)},uRain:{value:0},uBody:{value:SURF_BODY}};
...
const SURF_MAT=(function(){
  ...
  m.onBeforeCompile=function(sh){
    ...
    sh.fragmentShader='uniform float uTime;uniform float uAmp;uniform float uUnder;uniform float uSnell;uniform vec3 uWin;uniform vec3 uGlint;uniform float uRain;uniform float uBody;uniform float uChop;uniform mat3 uFogR;varying float vH;varying vec2 vWp;varying vec3 vNup;varying float vBrk;varying float vSpace;varying vec2 vDw;float snell=1.0;float cv2=1.0;float winT=0.0;uniform sampler2D uRefr;uniform float uRefrOn;uniform vec4 uRefrRes;uniform float uRefrK;uniform vec3 uLumC,uCirrC,uLum,uCam;uniform vec2 uWind,uUpper;uniform float uDay,uCover,uMoonL,uLumL,uCloudH,uCloudT,uCirrusH,uCirr,uFxCloud;\n'+SKY_NOISE_GLSL+CLOUD_GLSL+RIP_PARS+RIP_GLSL+WSHD_GLSL+WAVE_REST_GLSL+
