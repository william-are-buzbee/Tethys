...
const REFL_FACET=1.0,REFL_GRAZE=[0.12,0.35],FOAM_S=[0.17,0.27],WIN_T=0.7,WIN_LO=0.05,WIN_HI=0.8; // WIN_* (v11.50): the underside's translucency looking straight up (the sky's share of the pixel) and the band of the facet's cosine to the eye over which it goes from nothing to full — the naive window, no critical angle // FOAM_S (v11.46): the wind sea's slope (rad) at which a crest starts to whitecap and where the foam is whole — the three chop components sum to 0.22 in line in the trades, 0.35 shoaled; a calm's quarter chop never reaches it // REFL_GRAZE: the cosine (of the view to the mean surface) below which the Fresnel weight is the mean surface's and above which it is the facet's — 7° to 20° of depression
...
const surfaceU={uAmp:{value:WAVE_AMP},uUnder:{value:1},uSnell:{value:0},uWin:{value:new THREE.Vector3(1,1,1)},uGlint:{value:new THREE.Vector3(1.0,0.97,0.88)},uRain:{value:0},uBody:{value:SURF_BODY}};
// uUnder: 1 when the camera is under water; uWin: Snell's window's light relative to noon; uGlint: the sun's (or the moon's) colour for the refracted glint;
...
const SURF_MAT=(function(){
  ...
  m.onBeforeCompile=function(sh){
    sh.uniforms.uTime=timeU;sh.uniforms.uChop=chopU;sh.uniforms.uAmp=surfaceU.uAmp;sh.uniforms.uUnder=surfaceU.uUnder;sh.uniforms.uSnell=surfaceU.uSnell;sh.uniforms.uWin=surfaceU.uWin;sh.uniforms.uGlint=surfaceU.uGlint;sh.uniforms.uRain=surfaceU.uRain;sh.uniforms.uBody=surfaceU.uBody;sh.uniforms.uFogP={value:FOG_PSURF};for(let i=0;i<CAU_TEX.length;i++)sh.uniforms['uCauR'+i]={value:CAU_TEX[i].g};sh.uniforms.uCauG={value:CAU_GTEX};sh.uniforms.uWindOff=windOffU;for(const k of ['uLumC','uCirrC','uLum','uCam','uWind','uUpper','uDay','uCover','uMoonL','uLumL','uCloudH','uCloudT','uCirrusH','uCirr','uFxCloud'])sh.uniforms[k]=skyU[k]; // the clouds through the window (v11.49, CLOUD_GLSL): the sky's own uniform objects, shared // the window's sky (v11.43): the sky shader's own uniforms, shared. FOG_PSURF (v11.42): the surface is the boundary — its ray is fogged in the camera's medium whole (scene.js). v11.44: for one version this comment sat mid-line and ate the six bindings after it — the surface's fog set fell back to the split one and its own crests were classed under water: dark patches along every far crest from above
    sh.vertexShader='uniform float uTime;uniform sampler2D uFloorMap;attribute float aSpace;varying float vH;varying vec2 vWp;varying vec3 vNup;varying float vBrk;varying float vSpace;varying vec2 vDw;varying float vFoam;\n'+WAVE_DEP_GLSL+WAVE_GLSL+WAVE_SLOPE_GLSL+sh.vertexShader.replace('#include <begin_vertex>',
      '#include <begin_vertex>\n{vWp=transformed.xz+modelMatrix[3].xz;vec2 dw=waveDep(vWp,modelMatrix[3].y);float hh=waveH(vWp,uTime,aSpace,dw);transformed.y+=hh;vH=hh;vBrk=waveBrk(vWp,dw);vNup=normalMatrix*vec3(0.0,1.0,0.0);vSpace=aSpace;vDw=dw;'+
      'vec2 wd=vec2('+Math.cos(WIND_A).toFixed(5)+','+Math.sin(WIND_A).toFixed(5)+'),wq=vec2(-wd.y,wd.x);float f0=length(chopSlope(vWp,uTime,dw)),f1=length(chopSlope(vWp+wd*2.5,uTime,dw)),f2=length(chopSlope(vWp+wd*5.0,uTime,dw));'+
      'float f=max(smoothstep('+FOAM_S[0].toFixed(2)+','+FOAM_S[1].toFixed(2)+',f0),max(0.6*smoothstep('+FOAM_S[0].toFixed(2)+','+FOAM_S[1].toFixed(2)+',f1),0.3*smoothstep('+FOAM_S[0].toFixed(2)+','+FOAM_S[1].toFixed(2)+',f2)));'+
      'vec2 wc=floor(vec2(dot(vWp,wd)/6.0,dot(vWp,wq)/1.5));float hs=fract(sin(dot(wc,vec2(127.1,311.7)))*43758.5);vFoam=f*step(0.35,hs)*(0.6+0.4*hs);}'); // the foam by steepness (v11.46, WATER.md E): where the wind sea's slope (chopSlope: the short components, the physics' amplitude) passes FOAM_S the crest is breaking; the same test 2.5 and 5 m downwave is where the crest was a moment ago, so the foam trails on the back slope at 0.6 and 0.3 — a whitecap outlives its crest; broken into streaks by a hash on cells 6 m along the wind by 1.5 m across it. In a calm the chop's amplitude (uChop) is a quarter and the slope never reaches the threshold // the mesh sits at the tide (modelMatrix[3].y), so the water depth is the floor map's plus that (v11.44); vBrk: the breaking excess, the foam // vNup: the mean surface's normal (up) in view space — the Fresnel and the window are judged against it, not the facet (v11.6)
    sh.fragmentShader='uniform float uTime;uniform float uAmp;uniform float uUnder;uniform float uSnell;uniform vec3 uWin;uniform vec3 uGlint;uniform float uRain;uniform float uBody;uniform float uChop;uniform mat3 uFogR;varying float vH;varying vec2 vWp;varying vec3 vNup;varying float vBrk;varying float vSpace;varying vec2 vDw;varying float vFoam;float snell=1.0;float cv2=1.0;uniform vec3 uLumC,uCirrC,uLum,uCam;uniform vec2 uWind,uUpper;uniform float uDay,uCover,uMoonL,uLumL,uCloudH,uCloudT,uCirrusH,uCirr,uFxCloud;\n'+SKY_NOISE_GLSL+CLOUD_GLSL+RIP_PARS+RIP_GLSL+WSHD_GLSL+WAVE_REST_GLSL+
      ...
      .replace('#include <normal_fragment_begin>','#include <normal_fragment_begin>\n{vec3 V=normalize(vViewPosition);float cv=abs(dot(V,normal)),cm=abs(dot(V,normalize(vNup)));cv2=cv;'+
        ...
        'else{normal=-normal;snell=smoothstep(0.25,0.65,cv);float fo=max(vBrk,vFoam)*0.6;float trw=mix(exp(-uFogP.x*vFogDepth),exp(-uFogA.w*uFogA.w*vFogDepth*vFogDepth),uFogP.y);float winT=(uSnell>0.5)?0.0:smoothstep('+WIN_LO.toFixed(2)+','+WIN_HI.toFixed(2)+',cv)*'+WIN_T.toFixed(2)+'*trw*(1.0-0.3*uRain)*(1.0-fo);diffuseColor.rgb=mix(vec3(0.22,0.46,0.56)*0.3*(uSnell>0.5?snell:1.0),vec3(0.6,0.62,0.62),fo);diffuseColor.a=1.0-winT;}}') // the underside (v11.50, the person on v11.49's window: "like a snowglobe … I just want to look up and see what is going on above me, through a shader"): translucent by the facet's angle to the eye — the sky dome, drawn under water too now, and anything in the air show through by winT (WIN_T straight up, falling to nothing by WIN_LO of cos, × the water's own transmittance to the fragment, since the dome is not fogged; less under rain and foam) over the mirror's colour; each facet its own share, so the surface still moves. The physical window (v11.43–v11.49: Snell's cone, the critical angle, the mirror outside, the sun once) is the `snell window` row on the effects list (uSnell), off by default // the underside is opaque (v11.42.1): outside the window it is a total-internal-reflection mirror, inside it the window's own colour until WATER.md B draws the refracted sky. At 0.62–0.74 the shore and the sky behind it — fogged as air since v11.42 — bled through and flickered as the facets flipped (the person's video: a white flash at the water line); the black dome and the shimmer used to be what showed through
      .replace('#include <emissivemap_fragment>','#include <emissivemap_fragment>\nif(!gl_FrontFacing&&uUnder>0.5){vec3 rdw=normalize(vFogPos-uFogC);vec3 R=reflect(rdw,normalize(uFogR*normal));float fd=texture2D(uFloorMap,vFogPos.xz*'+WM_SCALE+'+0.5).r*'+FM_SCALE.toFixed(1)+';float L=min(uFogW.w,(vFogPos.y+fd)/max(-R.y,0.05));vec3 V=normalize(vViewPosition);vec3 Nd=normalize(mix(-normal,-normalize(vNup),0.5*uRain));vec3 Tv=refract(-V,Nd,1.33);vec3 Tw=dot(Tv,Tv)<0.5?normalize(vec3(rdw.x,0.02,rdw.z)):normalize(uFogR*Tv);vec3 sw=vec3(0.0);if(uSnell>0.5){sw=skyLite2(Tw,1.0,0.0);float wk=dot(Tv,Tv)<0.5?0.0:1.0;sw=mix(sw,uCirrC,cirrusA(Tw)*wk);vec4 cdk=cloudDeck(Tw,uSkA.xyz,uSkB.xyz);cdk.a*=wk;sw=mix(sw,cdk.rgb,cdk.a);vec3 Tm=refract(-V,-normalize(vNup),1.33);if(dot(Tm,Tm)>0.5){vec3 Tmw=normalize(uFogR*Tm);float csm=max(dot(Tmw,uSkE.xyz),0.0);sw+=uSkD.xyz*(pow(csm,300.0)*0.7+pow(csm,40.0)*0.25)*uSkA.w*(1.0-0.85*uSkB.w)*(1.0-cdk.a);float cmm=max(dot(Tmw,uSkF.xyz),0.0);sw+=vec3(0.95,0.93,0.85)*(0.15+0.85*min(1.0,uSkC.w*3.6))*(1.0-0.6*uSkA.w)*pow(cmm,400.0)*0.6*(1.0-0.9*uSkB.w)*(1.0-cdk.a);}sw=mix(sw,(uSkB.xyz*0.6+uSkA.xyz*0.4)*0.9,0.35*uRain);}float rk=uRain*(1.0-smoothstep(10.0,34.0,length(vViewPosition)));vec3 mir=fogVeil(vFogPos,R,L);totalEmissiveRadiance=mix(uSnell>0.5?mix(mir,sw*0.92,snell):mir,vec3(0.72,0.75,0.75)*uWin,max(vBrk,vFoam)*0.6)+vec3(0.80,0.86,0.90)*uWin*rainRing(vWp)*0.5*rk;}') // rain from below (v11.46, WATER.md K): the drops roughen the surface, so the eye's refraction leans toward the mean surface (Nd: the facets' flip matted by half) and the window toward the sky's mean (the sun's disc smeared), and the drops' rings and dots read as bright specks from beneath (rainRing, within ~30 m); the foam by steepness is a grey patch from below like the breaking (vFoam) // the window (v11.43, WATER.md B): the eye refracted through the facet into the air (eta 1.33; -normal faces the water) and the sky read in that direction; past the critical angle refract returns zero and the horizon's colour stands in, which the soft band blends into the mirror // the mirror is the veil in the reflected direction (v11.42.3, WATER.md C): the ray reflected off the facet, the water along it as far as the floor (uFloorMap) or the veil's reach, through the fog chunk's own fogVeil. To v11.42.2 it was fogColor·0.9, the CPU's veil at the camera — one flat colour with no daylight by direction and no sun — and with the underside opaque a camera at the line saw the near facets overhead as a dark slab against the sunlit water beside them
...
const CLOUD_GLSL=['float cirrusA(vec3 d){float up=d.y;float cv=0.0;',
  ...
  '    float n=fbm(cp)*0.72+0.28*fbm(cp*vec2(1.0,3.5)+vec2(2.3,7.1));float th=0.80-0.22*uCirr;cv=smoothstep(th,th+0.18,n)*0.45*smoothstep(0.012,0.09,up);}return cv;}',
  ...
  '    vec3 dayLit=mix(zen*2.2+hor*0.8,uLumC*1.3,clamp(uLumL*3.0,0.0,0.9)),nightLit=zen*1.6+hor*0.6+uLumC*uMoonL*0.30;vec3 litC=mix(nightLit,dayLit,uDay);',
  '    vec3 shdC=mix(mix(zen*0.9+hor*0.2,zen*0.6+hor*0.45,uDay),hor*0.5,uRain*0.5);float accA=0.0;vec3 accC=vec3(0.0);',
...
function updateAtmosphere(dt){
  ...
  surfaceU.uUnder.value=camUnder?1:0;surfaceU.uRain.value=FX.rain?K.rainA:0;surfaceU.uSnell.value=FX.snell?1:0;FOG_AC[3]=wlc;
