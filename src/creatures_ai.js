...
function softPrey(c,o){return kgOf(c)>=MOULT.prey*kgOf(o);} // whether c, a hunter, takes o, a soft body (findPrey)
...
function spawn(ch,kind,pos,rng,opt){
  ...
  c.mass=c.mass0=kgOfBody(kind,D,b); // one mass (v11.91, COMBAT.md §10.2): derive's, in kg, at this body's scale
...
function kill(c,by,whole){if(!c.alive)return;c.alive=false;c.target=null;c.grab=null;c.threat=null;c.scav=null;c.bleed=0;c.wounds=null;c.paraT=0;releaseAll(c);ecoDebit(c);POP.kills++;
...
function hungerTick(c,dt){const K=eaterK(c);c.hunger=Math.min(1,c.hunger+dt/(K.cycle*DAY_S));if(c.hunger>=1){c.starveT+=dt;if(c.starveT>K.cycle*DAY_S*STARVE_T){if(c===player)die('starved');else{POP.starved+=1;kill(c,null);}return true;}}return false;} // the player too (v11.73): the same clock, the same end
const STARVE_T=1.2; // cycles past hunger 1 before the body dies of it (the 1.2 of v11.26, named)
function updateHunter(c,dt){
  ...
  if(c.state==='chase'){
    ...
    else if(c.hold){c.vel.multiplyScalar(1-1.5*dt);c.biteT-=dt;c.grab=c.b.rigs?tg:null;c.commitT=0;c.coastT=0;c.strikeT=0;c.tellT=0;} // holding (v11.92, COMBAT.md §10.4): the pair drifts as one and the holder does not seek into a body it already has (that was the orbit); the bites, the shake, the roll and the claws' pull are the hold's (combat.js updateHolds, holderWork, v11.93)
    else if(d.strike){
...
function updateCoil(c,dt){
  ...
  if(c.state==='ram'){c.ramT-=dt;seek(c,player.pos,d.ram,dt,3);c.biteT-=dt;if(dist<reachOf(c,player)&&c.biteT<=0){c.biteT=2;blow(player,c);}if(c.ramT<=0||playerGone()){c.state='wander';c.cool=8;setWander(c);}}
...
function updateCreatures(dt0){
  ...
  for(const c of creatures){
    ...
    if(!d.noOrient){if(c.shook){_q.setFromAxisAngle(UP,-c.shook);c.g.quaternion.multiply(_q);c.shook=0;} // the shake (v11.93, combat.js holderWork): last frame's head yaw taken off before the facing is steered, and this frame's put on after — a yaw about the body's own up, composed on the facing, never fought by it
      if(c.face){_m.lookAt(c.face,c.pos,UP);_q.setFromRotationMatrix(_m);c.g.quaternion.slerp(_q,1-Math.exp(-(d.turn||2)*(c.turnK||1)*(c.commitT>0||c.strikeT>0?TURN_STRIKE:2)*dt));} // in a strike the head snaps to the point (TURN_STRIKE, v11.92): the mouth dips to a prey under it // turning to a thing (the tell, the watcher's stare)
      ...
      if(c.shakeYaw){_q.setFromAxisAngle(UP,c.shakeYaw);c.g.quaternion.multiply(_q);c.shook=c.shakeYaw;}}
  ...
  for(const c of near){c.g.updateMatrix();worldShapes(c);c.shapeF=frameNo;if((c.def.role==='ambush'&&c.state==='sit')||c.def.role==='trap'||c.dead)c.mass=1e6;else c.mass=c.mass0;}
