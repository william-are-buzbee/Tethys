...
**COMBAT.md §10 (24 Sep 2026): the physics pass, designed.** The person's report after hours as the finback (the petals' point clipping through faces and biting from a sphere, the orbit on impact, blood with no wound, blood not by the injury, no bleeding out, the hunter leaving after a tail) traced to the code, and the design: one mass in kg, the wound by the bite's radius against the part's, blood as a volume with a rate and a clot that kills at 40%, the mouth's contact instead of reach, the committed lunge and its overshoot, the hold as a joint at the struck point, the edge's work, stamina in the struggle. The person's answers are §10.9. **Next: pass A** (the wound and the blood). One open: whether the edge is a family trait (ridgebacks, longbacks) or a sub-clade.
