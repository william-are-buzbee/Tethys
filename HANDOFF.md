...
**v11.50.2 (15 Sep 2026): the white patch the shape of a light shaft.** The person's stills and clip: the far kelp crowns along the water line went white under a shaft, reliably after a new game, and only with `light shafts` on. The shafts were plain additive and four stacked ones clipped a pixel already at the pale veil. They blend as screen now (atmosphere.js `shM`, `dst + src·(1 − dst)`), so nothing can clip; no number moved. Seen at the person's readout spot (`test/render/fix_on.png`). Ask: the patch in play at that spot; the shafts over bright sand, which they brighten less now. Still open: the crowns read as a pale outline against the darker water behind them with or without shafts (the CHANGELOG entry says why).
