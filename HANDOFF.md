...
**v11.49 (14 Sep 2026): the sky through the window.** The person: little circles of light flickering when looking up, and no clouds. Both were v11.43's window as designed — the sun's disc read through every facet, and a reduced sky with the deck's mean shade in place of clouds. Now the dome's cumulus, scud and cirrus are shared strings (`CLOUD_GLSL`, `SKY_NOISE_GLSL`) the window draws along the refracted eye per facet, the disc and sharp glare are off per facet (`skyLite2`'s `dsc`) and the sun (and the moon) drawn once from the mean surface as one soft blob. Seen from 6 m and 3 m under at noon (`test/render/v49b_*.png`). **Ask first** for the render ms looking up under a full deck, then the sun blob's size, then the moon at night.
