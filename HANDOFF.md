...
**v11.93 (25 Sep 2026): COMBAT pass C — the edge's work.** The verdict is physics (combat.js `EDGE_K`, `COVER_T`, `thruK`): an edge is through a covering when its force (K × kg^⅔) × sharpness beats the covering's toughness × the capsule's radius, a crush against the radius squared; the `EDGE` table is gone and the same numbers say what any mouth at any size does (the eel's cut short of the finback's hide at rest, k 0.93; the ridge's 6.2; nothing but the crusher through a shell; the soft-arm's beak bleeds a grazer; claws pinch skin, not hide). The holder works its edge (`holderWork`): a cutting jaw shakes its head at its tail-beat's order (the grip swings the held; each cycle sharpens the saw to 3×), a chain body rolls on its axis with the held carried round it (the eel), claws holding a limb back off and tear it at the root when their force passes it (the sickle takes your tail in half a second; the hood never), a plain jaw clamps and waits. Subdued is by stamina, not a clock (`STAMINA` by clade, `SUBDUE`): the held body's struggle fades as it tires, and a second under half the grip is the pin. The swallow is a sequence (`SWALLOW`: 1 + 4 × the lengths' ratio seconds, the body drawn into the mouth, the death at the end). The arms reach only inside a cone (physics.js `ARM_CONE`, the person's note b). The mouth's edge at conception is priced as a line's step (line.js `BUDGET.edge` 1.5, the bill naming the sawmouths' step). test/combat.js is seeded now. On paper: the eel subdues the thrashing finback at 1 s, rolls, and bleeds it out at 14 s; the stone swallows it over 6.7 s; the trap, hood and hook let go of a hide. **v11.93.1:** the low tier's smoke test failed one run in three on v11.92 and v11.93 alike (a cell generated at NaN): the lab's placed creature had no `top` or `sprint` on its def and the v11.92 lunge multiplied them — fixed in lab.js, six runs clean. **Frozen here (the person, 25 Sep 2026):** other changes first in another chat (LODs, the FOV); pass D resumes in this one after their testing. Meanwhile the planet was designed in this chat: GLOBE.md — continents far away, a slow plate (the chain becomes a cluster with a seamount province), a pulsing plume, 10–20° (built already: LAT 0.2), Earth-like ice ages (a second shoreline at −120 on every member); the build (a planet map from rules, then a region window, then records into ISLANDS) is deferred to a chat with room for it. **Ask first** about the eel's roll with you in its jaws (`SPIN.rate`), then a ridge shaking a grazer (`SHAKE`), then being swallowed by a stone (`SWALLOW`), then the middle range with the grazer tiring (`STAMINA`), then the arm hunters angled away (`ARM_CONE`). **Next: pass D** — the predators' behaviour (commit to a crippled animal, follow the trail downcurrent, bite and wait on dangerous prey, break off when hurt, the return).

**v11.92 (24 Sep 2026): COMBAT pass B — contact, not reach; the lunge; the joint; the petals to the skin.** The person on A: "this pass looked good", go ahead. A jaw's bite is its mouth (a sphere of the gape at the mouth part, within 60° of the axis) touching a prey capsule — a slowblood beside you is nothing, one nose-on bites; arms and claws bite at their reach. A chase turns at the body's turn rate (a real turning circle; it brakes into tight turns and yaws to come round), aims where the prey will be, and in range with the prey ahead of its mouth commits: a locked lunge at the sprint with the head snapped to the point, the bite on contact, a miss coasting past. The impact shares momentum by mass; the hold is a joint on the prey's skin (no rope), the pair moving as one, the holder no longer seeking into what it holds (that was the orbit). The mouth's petals go for the point the hold has on the skin, inside the mouth's cone, never through the head. The stub turns creatures now (real `lookAt`/`slerp`), so headless fights mean more than they did. On paper: no orbit (the ortho sweeps 26° in 1.8 s, the ridge 0°), the joint never parts, a chasing ridge commits and a grazer that moves 2.5 m across is missed, a still one is taken in 1.3 s, the hunter table faces its hunters at you and reads: the eel bleeds you out in 14 s, the ridge/basker/abyssal/stone/crusher swallow, the lurker and the ortho take the nerve cord, the claws let go of a hide. The person's look (24 Sep 2026, eaten by an ortho): "AWESOME. terrifying."; the impact and the hold great as small prey; the orthos still orbit a little; the arm hunters angled away still contort, their limbs clipping lengthwise through their own bodies to reach (they should not push a reach outside a cone, as the mouth chains do not); the middle range — a small player grabbing a bigger animal — is awkward and unclear (constant detaching, an unreadable trajectory); tentacles should wrap a body, not reach for a point (a later pass). Recorded in COMBAT.md §10.9; pass C (v11.93) answers b and, in part, c.

**v11.91 (24 Sep 2026): COMBAT pass A — the wound by the bite, blood as a volume that kills, one mass.** COMBAT.md §10 is the person's report after hours as the finback (blood with no wound, blood not by the injury, no bleeding out, the hunter leaving after a tail; the petals' point and the orbit are pass B) with the design of passes A–D; A is built. One mass in kilograms (`kgOfBody`: derive's; the finback 1770, the ridge 45,450); every bite in a hold is the edge on the body where the hold is — nothing through the covering is a bruise with no blood; through, the bite's radius against the capsule's decides: a limb severed, the trunk opened to the vitals (the axis along the front; shallow at the nape behind the head), else a wound of its area; blood is a volume by clade, a wound drains it at a rate by its area and the part's vessels and clots on the clade's clock stretched by its size; 15% lost is weak, 30% collapsed (no sprint, no ability), 40% dead — "bled out"; a piece is the biter's food by its share, a body a quarter eaten is torn apart, and a hungry hunter keeps its target after a mouthful. §10.10: the edge's lineage — one program (the dermal plate rows on the petals) and one origin of the saw, the sawmouths (ridgebacks + longbacks; the basker moved to the ridgebacks in PLANET). On paper: the eel's flank bite 1.09%/s and the held finback dead of it at 16 s; a ridge severs the tail, the root pours, the ridge comes back; the tail alone bleeds you out in 39 s. **Ask first** whether a fight now reads as a wound and a bleed (the eel), then the crippled death's pace (`BLOOD.dead`, `BLOOD.k`, the root's vessel), then whether 16 s in an eel's jaws is right (`GRIP.k`), then the hingeshells (barely bleed, torn apart instead), then `TORN` 0.25.

**v11.90 (24 Sep 2026): the volume alone; the audio's 80× gain.** The person on v11.89: "absolutely awesome … a real sense of scale", remove the old system; almost too large; too dark; flat bottoms with a clear line; the audio "hilariously bass boosted". The bug: `Q.vol` is the master volume's key (v11.14) and v11.89's `vol:80` for the march's steps shadowed it in the same literal — 80× gain. Renamed `Q.march`. The slice deck, the scud and the lumps are struck with their switches; the low tier marches at 28. Smaller clouds (`CLD.sc` 1/750), lighter bases (the ambient 0.95–1.2 of a shade half way to light grey), rounded bottoms (`CLD.base` 0.12: the threshold rises again under it). Seen (`test/render/v90_*.png`): smaller cumulus in three tones, mid-grey bases, rounded undersides at sunset. **Ask first** whether the audio is back to its old level, then the size (`CLD.sc` is theirs), then the bases' tone and roundness, then the render ms on the 4060 (`Q.march`).

**v11.89 (24 Sep 2026): the cloud volume — CLOUDS.md §8.** The person, on the stills of v11.87–88: the lumps and the deck were two systems, one clearly 3D and one "a paper you fully color in", worse together; is there a foundational change avoided for the low-poly vibe? Built: the same field read as a true 3D density and ray-marched by the dome — the noise tiles (a period per octave), the GPU bakes it once into an atlas (32 slices of 256², half float), `cldDensT` gives the density at a world point with the local deck's depth, the caps and the shower cells, and `cloudVol` marches the slab with steps growing from 30 m from an ordered dither, two light samples toward the luminary a step, the sun's share averaged over the ray's cloud and quantised to three steps (the light banded after the cloud: terminator lines, not contours), the ambient by height, the silver lining, the fog by distance. (v11.90: the slices and the lumps are struck; the volume is the clouds on both tiers.) Seen (`test/render/v89d_*.png`, `v89e_*.png`): cumulus in three tones to the horizon, a cel-shaded volume overhead, backlit silhouettes and pink-lit faces at sunset, small cumulus on the clearest day, a shower's plateau with the shaft, the giant's cap from 300 m. **Ask first** for the readout's render ms above water on the 4060 with the switch on and off, then whether the three bands read as the world's own light, then the clouds' size (`CLD.sc`), then a cloud sliding over in play against the shadow on the sea and the light under water.

**v11.88 (24 Sep 2026): showers with a place — CLOUDS.md §6, pass D.** The person on v11.87: "Looks awesome. Go ahead on D." A shower is a cell (clouds.js `SHWR`, `updateCells`): the weather's rain is only the trigger; an episode births one cell 6 km upwind of the player on a hashed track across the wind (most pass beside you), steered along the wind at 2.2 × the trades, alive 13 real minutes; its congestus a bump in the cloud field (the deck thickens and deepens, a lump grows to a tower, the sea darkens under it), its rain at the camera by distance, a rain curtain under its base in the dome, the bow only on the curtain or in the rain here. Seen by hand-placed cells on the clearest day (`test/render/v88c_*.png`, `v88d_*.png`): the tower over the deck with the shaft under it, the rain over the camera, the bow over a cell at a low sun. **Ask first** whether a shower is seen coming across the water in play at its own pace and whether a quarter over you is right (`SHWR.u`, `SHWR.band`), then the curtain's read (`SHWR.vis`), then the tower lump (a towering cumulus or an iceberg), then the sea darkening ahead of the rain, then the two extra Gaussians' cost in every field read.

**v11.87 (24 Sep 2026): the clouds and the weather's clocks — CLOUDS.md passes A–C and the low-poly exploration.** The person asked whether the sky's cloud kinds were designed or accreted (designed, v11.17 — but the deck's dark patches were contour lines of one noise and the cirrus a field of dashes), what this planet's sky is by the day, the week and the year, and for as much variety as is believable. Built: the deck as one field written twice (clouds.js, GLSL and JavaScript from a float32-exact hash) — 3D billow noise, each slice its own shape, cloud streets along the trades, the tops leaning downwind, the threshold the cover's own quantile, a cap over the giant's summit that swells through the afternoon; the deck lit by the sun's real elevation in its own sunlight (amber undersides at sunset), a silver lining, the cirrus sparser and dark after sunset; the weather's clocks in `weatherAt` — showers and cover peaking before dawn and thinning by mid afternoon, easterly waves every 3–5 days, the cirrus the still season's, the congestus deeper and the haze whiter in it, the tops spreading flat toward dawn; the beam at the player by the cloud actually over it (full sun in a gap, shade as a cloud drifts over — the caustics and shadows follow), cloud shadows on the sea and the land (`Q.cldSh`, desktop only); and the near clouds as low-poly lumps at the field's maxima in the horizon pass, with the giant's cap as fixed lumps (`near clouds` on the effects list). Pass D (showers with a place) followed as v11.88. Seen (the loop driven by hand, `test/render/v87c_*.png`, `v87d_*.png`): the boot sky as streets of cumulus, a low-poly cumulus overhead, the broken deck at 0.62, the sunset with pink undersides, the clear day, the sea darker under a cloud from 40 m. `node test/sky.js` is new (the field's calibration, the year's statistics, the lumps' scan). **Ask first** whether the lumps read as clouds in motion at 1600×900 or want the switch off (CLOUDS.md §5 has the judgement so far), then the frame time above water with the deck overhead (the dome does ~400 hashes a pixel; `Q.cloud` 3 halves it) and on the sea (`Q.cldSh` 0 strips the shadows), then whether the light coming and going under a passing cloud reads right under water, then the sunset's undersides, then whether the streets read as rows or as stripes (`CLD.street`, `CLD.rowA`), then the cap on the giant from a height.
...
ring's box). The person's two notes from above the surface (22 Sep) are v11.82.1: the water's tint shows from the air now (`UPWELL` × the crop's tint at the
crossing) and the snow's top metres show through the surface. **Ask first** whether the snow's colour now sits in the dark water rather than on it, then whether the chains read as strings at swimming
...
whether the player is there); back the frame the window closes. **Ask first** whether the return beside something hungry wants a second of grace.
...
not at the hatch — mating is the trip to the creature editor, and changing nothing is allowed. **Grandchildren are struck**: only your own children
