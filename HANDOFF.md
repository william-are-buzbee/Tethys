...
**v11.50.3 (15 Sep 2026): the white patch the shape of a light shaft, found.** The person's stills and clip: the far kelp crowns along the water line lit white under a shaft, only with `light shafts` on; v11.50.2's screen blend did not touch it. The cause was the draw order: the shafts (−3) were drawn before the surface (−1 under water, writes depth), so the surface erased them over the far water and they survived only on the floor and the far crowns, which sit in front of it — the shaft's outline clipped to the crowns. The shafts draw at 0 now, after the surface; the screen blend stays. Seen at the person's readout spot (`test/render/ord_0.png`, `up2_0.png`). The person: "that worked perfectly". Ask: the shafts against the far water, which they now reach. Still open: the crowns read as a pale outline against the darker water behind them with or without shafts (CHANGELOG v11.50.2 says why).
