...
**v11.52.1 (15 Sep 2026): the pools partitioned by sight.** The person's readouts on v11.52: `work` halved, the vsync interval rose (16 ms in the air over the forest) — every loaded cell's flora was drawn, behind the camera too, and their GPU had no room for it; the pane's `gl.finish()` had under-read the GPU (a hidden tab never presents). Now the blocks of the cells in view come first in each pool and `im.count` is their total (chunks.js `poolCull`, `poolMove`; the shadow pass draws all). Triangles back to 4.7M at the spot with the draws still down (263 against 434). CHANGELOG v11.52.1. **Ask first** for the four readouts again — the interval should be 8.3 with `work` as on v11.52.
