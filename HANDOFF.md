...
**v11.36 (13 Sep 2026): the caustic is derived, and the sea it needs is baked.** The 13 Sep effects audit (the person's ask: every switch on `e`, bugs and fit with the low-poly style) found v11.13's caustic was a gain that clipped the sand white, at the facets' own scale, on its own clock. It is now `1/|det(I − d·c·H)|` of a ripple layer that lives only in the light (the swell can't focus in the top 30 m) — thirty-two trains in four wavelength rings baked at boot into two tiles a ring (`CAU_RINGS` … `CAU_TEX`, scene.js), read back exactly at any time from two taps and a sincos, riding the swell's orbital carry, blurred by the sun's disc per ring, drawn as pale lines where the focus reaches `CAU_T` 3; v11.36.1 the rings ×1.5 (3–0.65 m) after the person's "much better, still small dots"; v11.36.2 (14 Sep) the net drawn in 15 cm world-fixed blocks with a hard edge (`CAU_PX`, the person: "too clean for the low poly game") and the sun wider (`CAU_SUN` 0.03: the 25 m rock was as crisp as the sand). Named by the person's five shots and not built: a full net under the kelp canopy (the canopy term is the water map's 36 m texels; the fix is the world shadows), and the net's density being the same everywhere (a gusty modulation of the rings, a small bake). Five rounds with the person the same day (v11.35–v11.36: a lattice, a stripe band, a print, dots — per-fragment sines were never going to be a sea). `test/caustic.js` draws the field at any depth; tune there, the menu's white sand shows nothing. The person's 5 m and 16 m views are the first thing to ask about; `CAU_T`, `CAU_SOFT`, `t-y` are the knobs (CHANGELOG v11.36, Unseen). The audit's other findings are not built and stand as a list in the conversation of 13 Sep: the shadow map is never freed when `shadows` is off; `clouds` off leaves rain from a clear sky; the shimmer is a canvas radial gradient (the most foreign thing in the effects list); the soft shadow edge is the more foreign half of an otherwise in-style shadow.
